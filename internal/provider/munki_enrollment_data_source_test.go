...
func TestAccMunkiEnrollmentDataSource(t *testing.T) {
	name := acctest.RandString(12)
	tagName := acctest.RandString(12)
	resourceName := "zentral_munki_enrollment.test"
	dataSourceName := "data.zentral_munki_enrollment.test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMunkiEnrollmentDataSourceConfig(name, tagName),
...
func testAccMunkiEnrollmentDataSourceConfig(name string, tagName string) string {
	return fmt.Sprintf(`
...
data "zentral_munki_enrollment" "test" {
  id = zentral_munki_enrollment.test.id
}
`, name, tagName)
}
