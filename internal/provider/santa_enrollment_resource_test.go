...
func TestAccSantaEnrollmentResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create and Read
			{
				Config: testAccSantaEnrollmentResourceConfigBare(name),
			...
			// Update and Read
			{
				Config: testAccSantaEnrollmentResourceConfigFull(name, tagName, true),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrPair(
						resourceName, "configuration_id", cfgResourceName, "id"),
					resource.TestCheckResourceAttr(
			...
			{
				Config: testAccSantaEnrollmentResourceConfigFull(name, tagName, false),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(
						resourceName, "tag_ids.#", "0"),
					resource.TestCheckResourceAttr(
...
func testAccSantaEnrollmentResourceConfigBare(name string) string {
	return fmt.Sprintf(`
...
func testAccSantaEnrollmentResourceConfigFull(name string, tagName string, withSets bool) string {
	sets := ""
	if withSets {
