...
func TestAccMonolithEnrollmentDataSource(t *testing.T) {
	name := acctest.RandString(12)
	tagName := acctest.RandString(12)
	resourceName := "zentral_monolith_enrollment.test"
	dataSourceName := "data.zentral_monolith_enrollment.test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMonolithEnrollmentDataSourceConfig(name, tagName),
				Check: resource.ComposeAggregateTestCheckFunc(
					// Read by id
					resource.TestCheckResourceAttrPair(
						dataSourceName, "id", resourceName, "id"),
					resource.TestCheckResourceAttrPair(
						dataSourceName, "manifest_id", mResourceName, "id"),
...
func testAccMonolithEnrollmentDataSourceConfig(name string, tagName string) string {
	return fmt.Sprintf(`
...
data "zentral_monolith_enrollment" "test" {
  id = zentral_monolith_enrollment.test.id
}
`, name, tagName)
}
