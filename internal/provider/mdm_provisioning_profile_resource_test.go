...
//go:embed mdm_provisioning_profile_resource_test_data.b64
var dummyProvisioningProfileSource string

func TestAccMDMProvisioningProfileResource(t *testing.T) {
	name := acctest.RandString(12)
	aResourceName := "zentral_mdm_artifact.test"
	etResourceName := "zentral_tag.excluded"
	resourceName := "zentral_mdm_provisioning_profile.test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create and Read
			{
				Config: testAccMDMProvisioningProfileResourceConfigBare(name, dummyProvisioningProfileSource),
			...
			// Update and Read
			{
				Config: testAccMDMProvisioningProfileResourceConfigFull(name, dummyProvisioningProfileSource),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrPair(
						resourceName, "artifact_id", aResourceName, "id"),
					resource.TestCheckResourceAttr(
...
func testAccMDMProvisioningProfileResourceConfigBare(name string, source string) string {
	return fmt.Sprintf(`
...
resource "zentral_mdm_provisioning_profile" "test" {
  artifact_id = zentral_mdm_artifact.test.id
  source      = trimspace(%[2]q)
  version     = 1
  macos       = true
}
`, name, source)
...
func testAccMDMProvisioningProfileResourceConfigFull(name string, source string) string {
	return fmt.Sprintf(`
resource "zentral_mdm_artifact" "test" {
  name      = %[1]q
  type      = "Provisioning Profile"
