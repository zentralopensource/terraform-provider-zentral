...
func TestAccMonolithEnrollmentResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Update and Read
			{
				Config: testAccMonolithEnrollmentResourceConfigFull(name, tagName, true),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrPair(
						resourceName, "manifest_id", mResourceName, "id"),
					resource.TestCheckResourceAttrPair(
			...
			{
				Config: testAccMonolithEnrollmentResourceConfigFull(name, tagName, false),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(
						resourceName, "tag_ids.#", "0"),
					resource.TestCheckResourceAttr(
...
func testAccMonolithEnrollmentResourceConfigFull(name string, tagName string, withSets bool) string {
	sets := ""
	if withSets {
