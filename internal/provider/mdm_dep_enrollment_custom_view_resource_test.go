...
func TestAccMDMDEPEnrollmentCustomViewResource_basic(t *testing.T) {
			...
			// Create and Read
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrPair(
						resourceName, "dep_enrollment",
						"zentral_mdm_dep_enrollment.test", "id",
					),
					resource.TestCheckResourceAttrPair(
						resourceName, "custom_view",
						"zentral_mdm_enrollment_custom_view.test", "id",
					),
					// weight default expected to be 0 when not configured
